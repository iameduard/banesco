...
SELECT 
       ...
       processor,
       ROUND(PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY pct_processor_time),4) P95_PCT_PROCESSOR_TIME,
       ROUND(AVG(pct_processor_time),4) AVG_PCT_PROCESSOR_TIME,
       ROUND(PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY pct_user_time),4) P95_PCT_USER_TIME,
       ROUND(AVG(pct_user_time),4) AVG_PCT_USER_TIME
from bod_cpu
where substr(recording_time,13,2)='00' AND PROCESSOR='_Total' AND RECORDING_TIME IS NOT NULL AND CARGA='BODTRUNC01'--Selecciona la medicion de la hora..
group by 
CASE WHEN CARGA IN ('BODCOMPE124','BODUAP02','BODUAP03') THEN 'UAP' ELSE 
...
select 
       CASE WHEN REPLACE(CARGA,'.xlsx','') IN ('BODCOMPE124','BODUAP02','BODUAP03') THEN 'UAP' ELSE 
       CASE WHEN REPLACE(CARGA,'.xlsx','') IN ('BODTRUNC01','BODTRUNC02','BODTRUNC03','BODTRUNC04','BODTRUNC05','BODTRUNC11','BODTRUNC12') THEN 'SO' ELSE
       CASE WHEN REPLACE(CARGA,'.xlsx','') IN ('BODBDAPPS8') THEN 'bac y lbtr' ELSE 'OTRO' END END END SERVICIO,
       REPLACE(CARGA,'.xlsx','') SERVIDOR,
       case when substr(recording_time,1,3)='116' then substr(recording_time,2,2) else substr(replace(RECORDING_TIME,'/2016','/16'),7,2)  end yy,
       case when substr(recording_time,1,3)='116' then substr(recording_time,4,2) else substr(recording_time,1,2)  end mm,
       case when substr(recording_time,1,3)='116' then substr(recording_time,6,2) else substr(recording_time,4,2)  end dd,
       case when substr(recording_time,1,3)='116' then substr(recording_time,8,2) else substr(replace(RECORDING_TIME,'/2016','/16'),10,2)  end hh,
...
PHYSICAL_DISK_NAME,
DISK_NUMBER
ORDER BY SERVICIO,CARGA,YY,MM,DD,HH;
...
FROM BOD_METRICAS WHERE OBJETO='System';
...
ROUND(PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY CASE WHEN SPEEDOUT > 0 THEN ((8*BYTEOUT)/3600)/SPEEDOUT ELSE NULL END),6)*100 PCT_P95_UTIL_CANAL_OUT,
ROUND(PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY CASE WHEN SPEEDEIN > 0 THEN ((8*BYTEIN )/3600)/SPEEDEIN ELSE NULL END),6)*100 PCT_P95_UTIL_CANAL_IN
FROM BOD_COMPE_MET
